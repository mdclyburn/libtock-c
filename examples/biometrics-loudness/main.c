...
int main(void)
{
	...
	timer_in(2000, // << because that's when the first event is
			 global_timer_fired,
			 NULL,
			 &g_timer);
						...
						__attribute__ ((unused)) void* a4)
{
	...
	// schedule the next timer based on the smallest remainder.
	uint32_t least = 999999999;
	for (uint8_t i = 0; i < 3; i++)
	{
		if (g_next_firings[i] < least)
		{
			least = g_next_firings[i];
		}
	}
	timer_in(least,
			 global_timer_fired,
