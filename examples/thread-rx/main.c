...
#define BORDER_ROUTER_IP6_STR "fe80::4873:c036:da29:d88b"
...
uint32_t _g_timer_frequency;

int main(__attribute__((unused)) int argc, __attribute__((unused)) char* argv[]) {

	libtock_alarm_command_get_frequency(
		&_g_timer_frequency);
	printf("Timer frequency is %lu Hz\n",
		   _g_timer_frequency);
...
void initUdp(otInstance* aInstance) {
	...
	otUdpOpen(aInstance, &sUdpSocket, handle_packet_rx, aInstance);
	otUdpBind(aInstance, &sUdpSocket, &listenSockAddr, OT_NETIF_THREAD);
}

uint32_t t_start = 0;
uint32_t t_bck_init = 0;
uint32_t bck_idx = 0;
uint32_t bytes_rx[5];
	...
	const otMessageInfo* aMessageInfo)
{
	...
	if ((t_now - t_bck_init) > _g_timer_frequency) {
		t_bck_init = t_now;
		bck_idx = (bck_idx + 1) % 5;

		uint32_t total_window_bytes = 0;
		for (uint32_t i = 0; i < 5; i++) {
			total_window_bytes += bytes_rx[i];
		}

		printf("%lu,%lu\n",
			   (t_now - t_start) / _g_timer_frequency,
			   total_window_bytes / 5);

		bytes_rx[bck_idx] = 0;
	}

	const uint16_t message_len = otMessageGetLength(aMessage);
	bytes_rx[bck_idx] += message_len;
